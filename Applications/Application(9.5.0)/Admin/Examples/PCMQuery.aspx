<%@ Page Language="vb" AutoEventWireup="false" ValidateRequest="false" CodeBehind="PCMQuery.aspx.vb" Inherits="Dynamicweb.Admin.PCMQuery" %>
...
<head>
    ...
    <style>
        ...
        .select-box-option:not(.select-box-option-selected):hover {
            background-color: #eeeeee;
        }
...
<body class="screen-container">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="section-6">
                            <h2>PCM Query</h2>
                            <small class="card-header-subtitle">Query products</small>
                        </div>
                                    ...
                                    <div class="pcm-query-filter-group js-pcm-query-filter-group pcm-query-filter-group-focus group-selected">
                                        <div class="pcm-query-filter-item">
                                            ...
                                            <div class="pcm-query-filter-valueselector">
                                                <select class="js-multiselect" multiple>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function (event) {
            var selects = document.getElementsByClassName("js-multiselect");
            ...
            for (var i = 0; i < selects.length; i++) {
                ...
                label.classList.add('select-box-label');
                ...
                box.onclick = function (e) {
                    var elm = this.getElementsByClassName("select-box-options")[0];

                    if (elm.classList.contains('select-box-options-hidden')) {
                        elm.classList.remove('select-box-options-hidden');
                    } else {
                        elm.classList.add('select-box-options-hidden');
                    }
                };
            }
        });
        ...
        function fillVisibleLabel(box) {
            if (box.owner.selectedOptions.length > 3) {
            ...
            if (box.owner.selectedOptions.length <= 3) {
                ...
                for (var i = 0; i < box.owner.selectedOptions.length; i++) {
                    var tagitem = document.createElement('div');
                    tagitem.classList.add('select-box-label-tag');
                    tagitem.innerHTML = box.owner.selectedOptions[i].innerHTML;
                    box.firstChild.appendChild(tagitem);
                }
        ...
        function createItemslist(box) {
            var itemsBox = document.createElement('ul');
            itemsBox.classList.add('select-box-options');
            itemsBox.classList.add('select-box-options-hidden');
        ...
        function _setItemClasses(item, option) {
            item.classList.add('select-box-option');

            if (option.selected) {
                item.classList.add('select-box-option-selected');
            }
        }

        function _toggleItemSelection(e) {
            var item = e.target;
        ...
        function _setItemUnselected(item) {
            if (!item) return;
            item.classList.remove('select-box-option-selected');
