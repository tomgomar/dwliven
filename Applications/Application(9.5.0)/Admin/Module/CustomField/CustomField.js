...
function savePreviousSettings(settingsID) {
    ...
    var optionRows = dwGrid_CustomFieldOptions.rows.getAll();
    for (var i = 0; i < optionRows.length; i++) {
        settings.OptionsKeys[i] = optionRows[i].findControl("CustomFieldOptionKey").value;
        settings.OptionsValues[i] = optionRows[i].findControl("CustomFieldOptionValue").value;
    }
...
function loadNextSettings(settingsID) {
    ...
    // Add options to grid
    dwGrid_CustomFieldOptions.deleteRows(dwGrid_CustomFieldOptions.rows.getAll());
...
function handleRowSelected(obj, gridID) {
    ...
    if (row) {
        ...
        if (gridID != "CustomFieldOptions") {
            var types = row.findControl("CustomFieldTypeDrop")
            try {
...
function deleteSelectedRowFields(obj) {
    var row = dwGrid_CustomFieldsList.findContainingRow(obj); 
    dwGrid_CustomFieldsList.deleteRows([row]);
    $("options").hide();
}

function deleteSelectedRowOptions() {
    var row = dwGrid_CustomFieldOptions.rows.getSelected();
    dwGrid_CustomFieldOptions.deleteRows(row);
}
...
function help() {
    window.open('http://manual.net.dynamicweb.dk/Default.aspx?ID=1&m=keywordfinder&keyword=administration.managementcenter.customfields&LanguageID=' + helpLang, 'dw_help_window', 'location=no,directories=no,menubar=no,toolbar=yes,top=0,width=1024,height=' + (screen.availHeight - 100) + ',resizable=yes');
}

function ValidateSystemNames() {
    var rows = dwGrid_CustomFieldsList.rows.getAll();
    ...
    for (var i = 0; i < rows.length; i++) {
        var txtSysName = rows[i].findControl("CustomFieldSystemNameText");
        var hiddenSysName = rows[i].findControl("CustomFieldSystemName");
        if (hiddenSysName.value.length == 0) {
            var fLbl = rows[i].findControl("CustomFieldName");
    ...
    dwGrid_CustomFieldsList.deleteRows(emptyRows); //Remove all empty rows from the Grid.
    return hasTrouble;
}
