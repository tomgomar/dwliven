<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="EditDataGroup.aspx.vb" Inherits="Dynamicweb.Admin.EditDataGroup" %>
...
<head runat="server">
    ...
    <script type="text/javascript">        
        ...
        initiatePostBack = function (action, target) {
            var frm = document.getElementById("EditDataGroupForm");
            document.getElementById("PostBackAction").value = (action + ':' + target);
            frm.submit();
        }

        cancel = function () {
            parent.submitReload("DataGroup", '<%=CurrentDataGroupId%>');                        
        }

        deleteDataGroup = function () {
        ...
        if ('<%=Converter.ToBoolean(Dynamicweb.Context.Current.Request("refresh")) %>' == 'True') {
            parent.submitReload("DataGroup", '<%=Dynamicweb.Context.Current.Request("id")%>', '<%=Dynamicweb.Context.Current.Request("edit")%>');
        }
    </script>
</head>
<body class="screen-container">
    <form id="EditDataGroupForm" runat="server">
        <dwc:Card runat="server">
            <dwc:CardHeader runat="server" ID="lbHeader" Title="Edit data group"></dwc:CardHeader>
            <dw:Toolbar ID="ToolbarButtons" runat="server" ShowEnd="false" ShowAsRibbon="true">
                ...
                <dw:ToolbarButton ID="cmdDelete" runat="server" Divide="None" Icon="Delete" OnClientClick="deleteDataGroup()" Text="Delete" Visible="False" />
            </dw:Toolbar>
            <dwc:CardBody runat="server">
                <dw:GroupBox runat="server" Title="Data group information" ID="GroupBox1">
