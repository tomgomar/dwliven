...
function mouseoutBox(element) {
    $(element).childElements()[0].childElements()[0].childElements()[1].style.paddingLeft        = '5px'
    $(element).childElements()[0].childElements()[0].childElements()[0].style.width              = '20px';
    $(element).childElements()[0].childElements()[0].childElements()[0].style.height             = '20px';
    $(element).style.backgroundColor                                                             = '#FFFFFF';
    $(element).style.border                                                                      = 'none';
    $(element).childElements()[0].childElements()[0].childElements()[0].style.backgroundColor    = '#D4D0C8';
}
...
function mouseoverBox(element, pix) {
    ...
    $(element).childElements()[0].childElements()[0].childElements()[1].style.paddingLeft        = '5px'
    $(element).childElements()[0].childElements()[0].childElements()[0].style.width              = '19px';
    $(element).childElements()[0].childElements()[0].childElements()[0].style.height             = '18px';
    $(element).style.backgroundColor                                                             = '#C1D2EE';
    $(element).style.border                                                                      = 'solid 1px #316AC5';
    $(element).childElements()[0].childElements()[0].childElements()[0].style.backgroundColor    = '#99B7E3';
}
...
function selectLang(langID) {
	var sepr = '&';
	var oldURL = GetFileNameFromUrl(location.href).replace("#", "");
	var newURL = "";
...
function selectCountry(countryID) {
	...
	if (countryID == "-1") {
		location.href = "../Edit/EcomCountry_Edit.aspx?addAll=yes";
	} else {
		location.href = "../Edit/EcomCountry_Edit.aspx?autocreate="+ countryID;
	}
}		