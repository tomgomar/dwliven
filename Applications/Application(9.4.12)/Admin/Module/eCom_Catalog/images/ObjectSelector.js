...
function changeRowColor(objRow, ev) {
    ...
    var items = getElementsByClass("selected", document, "li");
    for (var i = 0; i < items.length; i++) {
        items[i].className = "dw-list-line";
    }
    
    if (!hasClass) {
        objRow.className = "dw-list-line selected";
    }
}
...
function addDivContent(itemId, itemName, itemImg, container) {
    var contentHolder = "";
    contentHolder += '  <li id="' + itemId + '" class="dw-list-line" onclick="changeRowColor(this,event);" title="' + itemName + '">';
    contentHolder += '    <i id="img' + itemId + '" class="' + itemImg + '"></i>';
    contentHolder += '    <span class="text">' + itemName + '</span> ';
    contentHolder += '  </li>';
...
function AddProdItemRows(id, radio) {
    ...
    for (var i = 0; i < products.length; i++) {
        addProductDivContent(products[i], products[i], 'fa fa-archive color-default', id + '_' + radio);
    }
...
function addProductDivContent(itemId, itemName, itemImg, container, itemObj, title) {
    var elem = new Element("li", {
        "id": itemId,
        "class": "dw-list-line",
        "onclick": "changeRowColor(this,event);",
        "title": itemName
    });
    ...
    contentHolder += '    <i id="img' + itemId + '" class="' + itemImg + '"></i>';
    contentHolder += '    <span class="text">' + itemName + '</span> ';
    elem.update(contentHolder)
    var CGdiv = document.getElementById(container + '_div');
    CGdiv.insert(elem);
    if (itemObj) {
        elem.setAttribute('itemData', JSON.stringify(itemObj));
    }
    if (title) {
...
function fillMultiProductsAndGroups(container, data) {
    ...
    for (i = 0; i < data.Items.length; i++) {
        ...
        if (item.Type == "p") {
            var lid = item.Id.lastIndexOf(",");
            var isVariant = lid > -1 && lid < item.Id.length - 1
            ...
            addProductDivContent(item.Id, item.Text, image, container, item);
        } else if (item.Type == "g") {
            addProductDivContent(item.Id, item.Text, groupImage, container, item);
        } else if (item.Type == "ss") {
            addProductDivContent(item.Id, item.Text, searchImage, container, item);
        }
    }
}
...
function CSVAddDropdownField(instanceId) {
    var div = document.getElementById('CSVFieldTypes' + instanceId);

    var newdiv = document.createElement('div');

    var select = document.createElement("select");
    select.name = instanceId + "_" + generateGuid();
    select.id = select.name;
    select.onfocus = function () { CSVSetSelectedElement(instanceId, this); }
    select.onchange = function () { CSVDropDownsUpdateHidden(instanceId); }

    for (index in optionsArray) {
        select.options[select.options.length] = new Option(optionsArray[index], index);
    }

    newdiv.appendChild(select);
    div.appendChild(newdiv);

    CSVDropDownsUpdateHidden(instanceId);
}

function CSVDeleteDropDownField(instanceId) {
    var fieldSelected = CSVGetSelectedElement(instanceId);

    if (fieldSelected != null) {
        if (confirm("Delete?")) {
            var d = document.getElementById('CSVFieldTypes' + instanceId);
...
function CSVMoveUp(instanceId, target) {
    CSVMove(instanceId, 'up', target);
}

function CSVMoveDown(instanceId, target) {
    CSVMove(instanceId, 'down', target);
}

function CSVMove(instanceId, action, target) {
    var fieldSelected = CSVGetSelectedElement(instanceId);
    if (fieldSelected != null) {
...
function CSVDropDownsUpdateHidden(instanceId) {

    var valueholder = document.getElementById(instanceId);
    valueholder.value = '';

    // Fields    
    var divholder = document.getElementById('CSVFieldTypes' + instanceId);
    var idholder = '';
    var inputs = divholder.getElementsByTagName('SELECT');
    ...
    var infoDiv = document.getElementById("CSVFieldTypes_Auto" + instanceId);
    infoDiv.style.display = inputs.length == 0 ? "block" : "none";
}
