...
dTree.prototype.node = function (node, nodeId) {
    ...
    if (this.config.useIcons) {
        var nodeIcon = node.knownIcon || "fa fa-folder-o color-folder";

        if (!node.icon) {
            node.icon = (this.root.id == node.pid) ? nodeIcon : ((node._hc) ? nodeIcon : this.icon.node);
        }
        if (!node.iconOpen) {
            node.iconOpen = (node._hc) ? node.knownIcon || "fa fa-folder-open-o color-folder" : this.icon.node;
        }
        if (!node.iconClose) {
            node.iconClose = (node._hc) ? nodeIcon : this.icon.node;
        }
