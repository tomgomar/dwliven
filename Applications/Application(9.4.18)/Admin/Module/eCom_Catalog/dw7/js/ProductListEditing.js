...
function ExpandTree(obj) {
    ...
    for (var i = 0 ; i < elements.length; i++) {
        if (i == 0) {
            var collapse = elements[i].attributes.getNamedItem("__collapse").value == 'false' ? 'true' : 'false';
            elements[i].removeClassName(collapse == 'true' ? "variable-product-minus" : "variable-product-plus");
            elements[i].addClassName(collapse == 'true' ? "variable-product-plus" : "variable-product-minus");
            elements[i].attributes.getNamedItem("__collapse").value = collapse;
            continue;
        }
