    ...
    function TilesView($me) {
        ...
        var createNode = function (listCnt, opts, nodeObj, count) {
            ...
            if (nodeObj.DefaultAction) {
                var act = "Action.Execute(" + JSON.stringify(nodeObj.DefaultAction) + ", " + JSON.stringify(nodeObj) + ");";
                itemWrap.append($("<a onclick='" + act + "'>").html('<div class="tile-image">' + icon + '</div><div class="tile-text">' + nodeObj.Title + '</div>'));
            } else {
