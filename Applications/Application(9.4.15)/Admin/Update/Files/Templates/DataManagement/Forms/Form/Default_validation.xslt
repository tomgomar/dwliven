...
<xsl:stylesheet version="1.0"  xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match ="/Template">		
		...
		<form method="post" id="{concat('FormSave', Form.ID)}" name="{concat('FormSave', Form.ID)}" enctype="multipart/form-data" action="{concat('/Default.aspx?ID=', GlobalTags/Global.Page.ID)}" onsubmit="return formValidation.validate(this);">
			<xsl:apply-templates select="Form.Hidden" />
			<xsl:apply-templates select="loop[@name='Form.Fields'][item]" />
			<xsl:value-of select="Form.Button.Submit" disable-output-escaping="yes" />
		</form>
	</xsl:template>
	...
	<xsl:template match="Field.Type[.='TextInput']">
		...
		<xsl:if test="string-length(../Field.ValidationValue) != 0">
			<input type="hidden" name="__validate" value="{concat(../Field.Systemname, '|', ../Field.ValidationValue)}" />
		</xsl:if>
	</xsl:template>
