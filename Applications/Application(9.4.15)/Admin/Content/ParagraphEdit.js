...
Dynamicweb.Paragraph.ItemEdit.prototype.initialize = function (options) {
    ...
    if (options && options.masterItem && Array.isArray(options.masterItem)) {
        for (var i = 0; i < options.masterItem.length; i++) {
            ...
            var itemControl = itemField.fieldType === "UserEditor" ? $(itemField.fieldName + "hidden") : $$('[name="' + itemField.fieldName + '"]')[0];
            ...
            itemControl.up(".item-field-value-container").previous("td").select("span")[0].appendChild(container);
        }
    }
}
